...
MCP-сервер для Cursor Desktop. Он запускает цикл кликов [jev-browser-use](https://github.com/wy-coliney/jev-browser-use) в Google Chrome. Cursor ставит задачу, вводит текст и проверяет результат. Jev выбирает следующий клик.

Этот репозиторий не копирует скилл и не является его форком. Сервер импортирует установленный `bridge.mjs`. Скилл под лицензией MIT. Этот проект — отдельный адаптер для Cursor, а не официальная часть скилла.
